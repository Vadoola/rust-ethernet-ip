...
mod udt_enhanced_parsing_tests {
    ...
    async fn test_udt_chunked_reading() {
        ...
        match result {
            Ok(PlcValue::Udt(udt_data)) => {
                println!("✅ Chunked UDT read successfully with {} members", udt_data.len());
                // Part_Data might be empty due to parsing limitations, but should not fail
            }
            Ok(other) => {
                println!("✅ Chunked UDT read successfully (different type): {:?}", other);
            }
            Err(e) => {
    ...
    async fn test_udt_gTracking() {
        ...
        match result {
            Ok(PlcValue::Udt(udt_data)) => {
                println!("✅ gTracking UDT read successfully with {} members", udt_data.len());
            ...
            Ok(other) => {
                println!("✅ gTracking read successfully (different type): {:?}", other);
            }
            Err(e) => {
    ...
    async fn test_udt_parsing_performance() {
        ...
        match result {
            Ok(_) => {
                ...
                assert!(duration < Duration::from_millis(100), "UDT parsing should be fast");
            }
            Err(e) => {
                if e.to_string().contains("Connection") || e.to_string().contains("timeout") {
                    println!("⚠️ Skipping test - PLC not available");
                ...
                for (key, value) in &udt_data {
                    match value {
                        PlcValue::Dint(dint_val) => {
                            assert!(*dint_val >= -1000000 && *dint_val <= 1000000, 
                                "DINT value {} for {} should be reasonable", dint_val, key);
                        }
                        PlcValue::Real(real_val) => {
                            assert!(*real_val >= -1000.0 && *real_val <= 1000.0, 
                                "REAL value {} for {} should be reasonable", real_val, key);
                        }
                        _ => {
