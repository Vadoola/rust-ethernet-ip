...
use rust_ethernet_ip::udt::{UserDefinedType, UdtMember, UdtDefinition};
...
impl MockEipClient {
    ...
    async fn read_tag(&mut self, tag_name: &str) -> Result<PlcValue, Box<dyn std::error::Error>> {
        if tag_name.ends_with("UDT") || tag_name == "Part_Data" || tag_name == "MyUDT" || tag_name == "TestUDT" {
    ...
    async fn read_udt_chunked(&mut self, tag_name: &str) -> Result<PlcValue, Box<dyn std::error::Error>> {
        if tag_name.ends_with("UDT") || tag_name == "Part_Data" || tag_name == "MyUDT" || tag_name == "TestUDT" {
    ...
    async fn read_udt_member(&mut self, udt_name: &str, member_name: &str) -> Result<PlcValue, Box<dyn std::error::Error>> {
        if udt_name.ends_with("UDT") || udt_name == "Part_Data" || udt_name == "MyUDT" || udt_name == "TestUDT" {
            let udt = create_test_udt_definition();
    ...
    async fn write_udt_member(&mut self, udt_name: &str, member_name: &str, value: PlcValue) -> Result<(), Box<dyn std::error::Error>> {
        if udt_name.ends_with("UDT") || udt_name == "Part_Data" || udt_name == "MyUDT" || udt_name == "TestUDT" {
            let udt = create_test_udt_definition();
...
fn create_test_udt_definition() -> UserDefinedType {
    ...
    udt.add_member(UdtMember { name: "bool1".to_string(), data_type: 0x00C1, offset: 0, size: 1 });
    udt.add_member(UdtMember { name: "int1".to_string(), data_type: 0x00C2, offset: 2, size: 2 });
    udt.add_member(UdtMember { name: "dint1".to_string(), data_type: 0x00C4, offset: 4, size: 4 });
    udt.add_member(UdtMember { name: "real1".to_string(), data_type: 0x00CA, offset: 8, size: 4 });
    udt.add_member(UdtMember { name: "string1".to_string(), data_type: 0x00CE, offset: 12, size: 84 });
...
async fn test_udt_data_type_parsing() {
    ...
    // Test BOOL parsing
    let bool_member = UdtMember { name: "test_bool".to_string(), data_type: 0x00C1, offset: 0, size: 1 };
    let bool_value = udt.parse_member_value(&bool_member, &test_data[0..1]).unwrap();
    assert_eq!(bool_value, PlcValue::Bool(true));
    
    // Test REAL parsing
    let real_member = UdtMember { name: "test_real".to_string(), data_type: 0x00CA, offset: 0, size: 4 };
    let real_value = udt.parse_member_value(&real_member, &test_data[8..12]).unwrap();
    assert_eq!(real_value, PlcValue::Real(3.14159));
    
    // Test STRING parsing
    let string_member = UdtMember { name: "test_string".to_string(), data_type: 0x00CE, offset: 0, size: 84 };
    let string_value = udt.parse_member_value(&string_member, &test_data[12..96]).unwrap();
...
async fn test_udt_data_type_serialization() {
    ...
    let bool_member = UdtMember { name: "test_bool".to_string(), data_type: 0x00C1, offset: 0, size: 1 };
    let bool_data = udt.serialize_member_value(&bool_member, &PlcValue::Bool(true)).unwrap();
    assert_eq!(bool_data, vec![1]);
    
    // Test REAL serialization
    let real_member = UdtMember { name: "test_real".to_string(), data_type: 0x00CA, offset: 0, size: 4 };
    let real_data = udt.serialize_member_value(&real_member, &PlcValue::Real(3.14)).unwrap();
    assert_eq!(real_data, 3.14f32.to_le_bytes().to_vec());
    
    // Test STRING serialization
    let string_member = UdtMember { name: "test_string".to_string(), data_type: 0x00CE, offset: 0, size: 84 };
    let string_data = udt.serialize_member_value(&string_member, &PlcValue::String("Hello".to_string())).unwrap();
...
async fn test_udt_member_writing() {
    ...
    udt.write_member(&mut test_data, "bool1", &PlcValue::Bool(false)).unwrap();
    let bool_value = udt.read_member(&test_data, "bool1").unwrap();
    assert_eq!(bool_value, PlcValue::Bool(false));
    
    // Test writing REAL member
    udt.write_member(&mut test_data, "real1", &PlcValue::Real(99.9)).unwrap();
    let real_value = udt.read_member(&test_data, "real1").unwrap();
    assert_eq!(real_value, PlcValue::Real(99.9));
    
    // Test writing STRING member
    udt.write_member(&mut test_data, "string1", &PlcValue::String("NEW123".to_string())).unwrap();
    let string_value = udt.read_member(&test_data, "string1").unwrap();
...
async fn test_udt_hash_map_conversion() {
    ...
    assert_eq!(hash_map.get("string1"), Some(&PlcValue::String("Hello UDT!".to_string())));
...
async fn test_udt_chunked_reading() {
    ...
    if let Ok(PlcValue::Udt(udt_data)) = result {
        ...
        assert_eq!(udt_data.get("string1"), Some(&PlcValue::String("Hello UDT!".to_string())));
    }
...
async fn test_udt_member_writing_client() {
    ...
    client.write_udt_member("TestUDT", "bool1", PlcValue::Bool(false)).await.unwrap();
    let bool_value = client.read_udt_member("TestUDT", "bool1").await.unwrap();
    assert_eq!(bool_value, PlcValue::Bool(false));
    
    client.write_udt_member("TestUDT", "real1", PlcValue::Real(99.9)).await.unwrap();
    let real_value = client.read_udt_member("TestUDT", "real1").await.unwrap();
    assert_eq!(real_value, PlcValue::Real(99.9));
    
    client.write_udt_member("TestUDT", "string1", PlcValue::String("NEW123".to_string())).await.unwrap();
    let string_value = client.read_udt_member("TestUDT", "string1").await.unwrap();
    assert_eq!(string_value, PlcValue::String("NEW123".to_string()));
    
    // Test writing to non-existent member
    let result = client.write_udt_member("TestUDT", "NonExistentMember", PlcValue::Bool(true)).await;
    assert!(result.is_err());
    
    // Test writing to non-existent UDT
    let result = client.write_udt_member("NonExistentUDT", "bool1", PlcValue::Bool(true)).await;
    assert!(result.is_err());
}
...
async fn test_udt_comprehensive_workflow() {
    ...
    client.write_udt_member("TestUDT", "bool1", PlcValue::Bool(false)).await.unwrap();
    client.write_udt_member("TestUDT", "real1", PlcValue::Real(99.9)).await.unwrap();
    client.write_udt_member("TestUDT", "string1", PlcValue::String("UPDATED123".to_string())).await.unwrap();
...
async fn test_udt_performance() {
    ...
    assert!(duration.as_millis() < 100, "UDT parsing too slow: {:?}", duration);
    ...
    assert!(duration.as_millis() < 50, "UDT member access too slow: {:?}", duration);
}
...
async fn test_udt_edge_cases() {
    ...
    let string_member = UdtMember { name: "test_string".to_string(), data_type: 0x00CE, offset: 0, size: 84 };
    let string_data = udt.serialize_member_value(&string_member, &PlcValue::String(max_string)).unwrap();
    ...
    let string_data = udt.serialize_member_value(&string_member, &PlcValue::String(too_long_string)).unwrap();
