...
impl<'a> TagPathParser<'a> {
    ...
    fn parse_program_scoped(&mut self) -> Result<TagPath> {
        ...
        let mut path = TagPath::Program { program_name, tag_name };
    ...
    fn parse_qualifier(&mut self, base_path: TagPath) -> Result<TagPath> {
        match self.peek_char() {
            ...
            _ => Err(EtherNetIpError::Protocol(
                format!("Unexpected character at position {}", self.position)
            )),
        }
    }
    ...
    fn parse_member_or_bit_access(&mut self, base_path: TagPath) -> Result<TagPath> {
        ...
        if let Ok(bit_index) = identifier.parse::<u8>() {
            if bit_index < 32 { // Valid bit range for DINT
    ...
    fn parse_number(&mut self) -> Result<u32> {
        ...
        self.input[start..self.position].parse()
...
mod tests {
    ...
    fn test_controller_scoped_tag() {
        ...
        assert_eq!(path, TagPath::Controller {
    ...
    fn test_program_scoped_tag() {
        ...
        assert_eq!(path, TagPath::Program {
    ...
    fn test_array_access() {
        ...
        if let TagPath::Array { base_path, indices } = path {
            assert_eq!(*base_path, TagPath::Controller {
    ...
    fn test_bit_access() {
        ...
        if let TagPath::Bit { base_path, bit_index } = path {
            assert_eq!(*base_path, TagPath::Controller {
    ...
    fn test_member_access() {
        ...
        if let TagPath::Member { base_path, member_name } = path {
            assert_eq!(*base_path, TagPath::Controller {
    ...
    fn test_complex_nested_path() {
        ...
        if let TagPath::Bit { base_path, bit_index } = path {
            ...
            if let TagPath::Member { base_path, member_name } = *base_path {
                assert_eq!(member_name, "Status");
                
                if let TagPath::Array { base_path, indices } = *base_path {
                    assert_eq!(indices, vec![2]);
                    
                    if let TagPath::Program { program_name, tag_name } = *base_path {
