...
async fn main() -> Result<(), Box<dyn Error>> {
                ...
                match client.read_tag(tag_name).await {
                    Ok(PlcValue::String(read_value)) => {
                        if read_value == *test_value {
                            ...
                            println!("❌ VERIFICATION FAILED: Expected '{}', got '{}'", 
                                   test_value, read_value);
                            println!("   Expected length: {}, Actual length: {}", 
                                   test_value.len(), read_value.len());
                        }
                    }
                    Ok(other) => {
                        println!("❌ VERIFICATION FAILED: Read returned wrong type: {:?}", other);
                    }
                    Err(e) => {
    ...
    match client.write_tag(tag_name, PlcValue::String(special_test.to_string())).await {
        Ok(_) => {
            ...
            match client.read_tag(tag_name).await {
                Ok(PlcValue::String(read_value)) => {
                    if read_value == special_test {
                        ...
                        println!("❌ Special character verification failed: '{}' != '{}'", 
    ...
    match client.write_tag(tag_name, PlcValue::String(number_test.to_string())).await {
        Ok(_) => {
            ...
            match client.read_tag(tag_name).await {
                Ok(PlcValue::String(read_value)) => {
                    if read_value == number_test {
                        ...
                        println!("❌ Numeric string verification failed: '{}' != '{}'", 
