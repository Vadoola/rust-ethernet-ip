...
async fn demonstrate_tag_path_parsing() -> Result<(), Box<dyn Error>> {
    ...
    println!("   Simple tag: {} -> {}", simple_tag, simple_tag.to_string());
...
async fn demonstrate_real_plc_operations(client: &mut EipClient) -> Result<(), Box<dyn Error>> {
    ...
    // Read array element
    match client.read_tag("Program:MainProgram.SensorReadings[0]").await {
    ...
    match client.write_tag("Program:MainProgram.SetPoint", PlcValue::Dint(1500)).await {
        Ok(_) => println!("   ✅ Set point updated to 1500"),
        Err(e) => println!("   ❌ Error writing set point: {}", e),
    }
    
    // Write to array element
    match client.write_tag("Program:MainProgram.Setpoints[0]", PlcValue::Real(72.5)).await {
        Ok(_) => println!("   ✅ Setpoint[0] updated to 72.5"),
        Err(e) => println!("   ❌ Error writing setpoint: {}", e),
    }
    
    // Write to bit
    match client.write_tag("Program:MainProgram.ControlWord.3", PlcValue::Bool(true)).await {
...
async fn demonstrate_performance() -> Result<(), Box<dyn Error>> {
    ...
    println!("   Parsed {} paths in {:?}", 1000 * test_paths.len(), duration);
