...
async fn demonstrate_data_types() -> Result<(), Box<dyn Error>> {
    ...
    println!("   True:  {:?} -> CIP Type: 0x{:04X}", bool_true, bool_true.get_data_type());
    println!("   False: {:?} -> CIP Type: 0x{:04X}", bool_false, bool_false.get_data_type());
    
    // 2. Signed Integer Types
    println!("\n2. Signed Integer Types:");
    
    let sint_val = PlcValue::Sint(-128);
    println!("   SINT:  {:?} -> CIP Type: 0x{:04X} (Range: -128 to 127)", 
             sint_val, sint_val.get_data_type());
    
    let int_val = PlcValue::Int(-32768);
    println!("   INT:   {:?} -> CIP Type: 0x{:04X} (Range: -32,768 to 32,767)", 
             int_val, int_val.get_data_type());
    
    let dint_val = PlcValue::Dint(-2147483648);
    println!("   DINT:  {:?} -> CIP Type: 0x{:04X} (Range: -2,147,483,648 to 2,147,483,647)", 
             dint_val, dint_val.get_data_type());
    ...
    println!("   USINT: {:?} -> CIP Type: 0x{:04X} (Range: 0 to 255)", 
             usint_val, usint_val.get_data_type());
    
    let uint_val = PlcValue::Uint(65535);
    println!("   UINT:  {:?} -> CIP Type: 0x{:04X} (Range: 0 to 65,535)", 
             uint_val, uint_val.get_data_type());
    
    let udint_val = PlcValue::Udint(4294967295);
    println!("   UDINT: {:?} -> CIP Type: 0x{:04X} (Range: 0 to 4,294,967,295)", 
             udint_val, udint_val.get_data_type());
    
    let ulint_val = PlcValue::Ulint(18446744073709551615);
    println!("   ULINT: {:?} -> CIP Type: 0x{:04X} (Range: 0 to 18,446,744,073,709,551,615)", 
             ulint_val, ulint_val.get_data_type());
    
    // 4. Floating Point Types
    println!("\n4. Floating Point Types:");
    
    let real_val = PlcValue::Real(123.456);
    println!("   REAL:  {:?} -> CIP Type: 0x{:04X} (32-bit IEEE 754)", 
             real_val, real_val.get_data_type());
    
    let lreal_val = PlcValue::Lreal(123.456789012345);
    println!("   LREAL: {:?} -> CIP Type: 0x{:04X} (64-bit IEEE 754)", 
             lreal_val, lreal_val.get_data_type());
    
    // 5. String Type
    println!("\n5. String Type:");
    let string_val = PlcValue::String("Hello, PLC!".to_string());
    println!("   STRING: {:?} -> CIP Type: 0x{:04X}", 
             string_val, string_val.get_data_type());
    ...
    println!("   UDT: Motor Data -> CIP Type: 0x{:04X}", udt_val.get_data_type());
...
async fn demonstrate_real_plc_data_types(client: &mut EipClient) -> Result<(), Box<dyn Error>> {
    ...
    match client.write_tag("TestLint", PlcValue::Lint(-9000000000000000000)).await {
    ...
    match client.write_tag("TestUdint", PlcValue::Udint(3000000000)).await {
        Ok(_) => println!("   ✅ UDINT written successfully"),
        Err(e) => println!("   ❌ UDINT write failed: {}", e),
    }
    
    match client.write_tag("TestUlint", PlcValue::Ulint(15000000000000000000)).await {
    ...
    match client.write_tag("TestLreal", PlcValue::Lreal(123.456789012345)).await {
    ...
    let tags_to_read = vec![
        "TestBool", "TestSint", "TestInt", "TestDint", "TestLint",
        "TestUsint", "TestUint", "TestUdint", "TestUlint",
        "TestReal", "TestLreal", "TestString"
    ];
...
async fn demonstrate_data_type_validation() -> Result<(), Box<dyn Error>> {
    ...
    println!("   LREAL small: {:?}, large: {:?}", lreal_small, lreal_large);
