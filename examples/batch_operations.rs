...
use rust_ethernet_ip::{EipClient, BatchOperation, BatchConfig, PlcValue, BatchError};
...
async fn demo_batch_reads() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("📊 Reading {} sensor values in a single batch operation...", sensor_tags.len());
    ...
    println!("🔍 Performance: {} tags read in a single network operation", sensor_tags.len());
    println!("   vs {} separate network calls with individual reads\n", sensor_tags.len());
...
async fn demo_batch_writes() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("🎯 Writing {} setpoints in a single batch operation...", setpoints.len());
    ...
    println!("🚀 Performance: {} setpoints written atomically", setpoints.len());
...
async fn demo_mixed_batch() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let operations = vec![
        ...
        BatchOperation::Read { tag_name: "Tank_Level".to_string() },
        BatchOperation::Read { tag_name: "Tank_Temperature".to_string() },
        BatchOperation::Read { tag_name: "Pump_Status".to_string() },
        BatchOperation::Read { tag_name: "Emergency_Stop".to_string() },
    ...
    println!("🎭 Executing mixed batch: {} reads + {} writes...", 
             operations.iter().filter(|op| matches!(op, BatchOperation::Read { .. })).count(),
             operations.iter().filter(|op| matches!(op, BatchOperation::Write { .. })).count());
...
async fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("   Individual operations: {:?} ({:.1}ms per tag)", 
             individual_elapsed, 
             individual_elapsed.as_millis() as f64 / tag_count as f64);
    println!("   Batch operation:       {:?} ({:.1}ms per tag)", 
             batch_elapsed,
    ...
    println!("   🏆 Speedup: {:.1}x faster with batch operations!", speedup);
    println!("   📡 Network efficiency: {} packets vs 1 packet", tag_count);
...
async fn demo_advanced_configuration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("   - Max operations per packet: {}", high_perf_config.max_operations_per_packet);
    println!("   - Max packet size: {} bytes", high_perf_config.max_packet_size);
    println!("   - Timeout: {}ms", high_perf_config.packet_timeout_ms);
    println!("   - Continue on error: {}", high_perf_config.continue_on_error);
    println!("   - Optimize packing: {}", high_perf_config.optimize_packet_packing);
    ...
    println!("   - Max operations per packet: {}", conservative_config.max_operations_per_packet);
    println!("   - Max packet size: {} bytes", conservative_config.max_packet_size);
    println!("   - Timeout: {}ms", conservative_config.packet_timeout_ms);
    println!("   - Continue on error: {}", conservative_config.continue_on_error);
    println!("   - Optimize packing: {}", conservative_config.optimize_packet_packing);
    ...
    println!("   - Max operations per packet: {}", balanced_config.max_operations_per_packet);
    println!("   - Max packet size: {} bytes", balanced_config.max_packet_size);
    println!("   - Timeout: {}ms", balanced_config.packet_timeout_ms);
    println!("   - Continue on error: {}", balanced_config.continue_on_error);
    println!("   - Optimize packing: {}", balanced_config.optimize_packet_packing);
...
async fn demo_error_handling() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let operations = vec![
        BatchOperation::Read { tag_name: "ValidTag1".to_string() },
        BatchOperation::Read { tag_name: "InvalidTag".to_string() },  // Will fail
        BatchOperation::Read { tag_name: "ValidTag2".to_string() },
        BatchOperation::Write { 
...
async fn demo_real_world_scenarios() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let manufacturing_tags = [
        "Station1_PartPresent", "Station1_CycleTime", "Station1_Quality",
        "Station2_PartPresent", "Station2_CycleTime", "Station2_Quality", 
        "Station3_PartPresent", "Station3_CycleTime", "Station3_Quality",
        "Station4_PartPresent", "Station4_CycleTime", "Station4_Quality",
        "Station5_PartPresent", "Station5_CycleTime", "Station5_Quality",
        "ConveyorSpeed", "ProductionCount", "RejectCount",
    ];
    
    let mfg_results = simulate_batch_read_results(&manufacturing_tags);
    println!("   ✅ Read {} manufacturing parameters in single operation", mfg_results.len());
    ...
    println!("   ✅ Updated {} HVAC parameters atomically", hvac_results.len());
    ...
    let safety_tags = [
        "EmergencyStop_1", "EmergencyStop_2", "EmergencyStop_3",
        "LightCurtain_1", "LightCurtain_2", "PressureSafety",
        "TempSafety_Zone1", "TempSafety_Zone2", "GasDetector",
        "FireAlarm", "SmokeDetector", "VibrationAlarm",
    ];
    
    let safety_results = simulate_batch_read_results(&safety_tags);
    println!("   ✅ Monitored {} safety systems rapidly", safety_results.len());
    ...
    let energy_tags = [
        "MainPower_kW", "MainPower_kVAR", "MainPower_PF",
        "Line1_Power", "Line2_Power", "Line3_Power",
        "HVAC_Power", "Lighting_Power", "Compressor_Power",
        "UPS_Load", "Generator_Status", "PeakDemand",
    ];
    
    let energy_results = simulate_batch_read_results(&energy_tags);
    println!("   ✅ Collected {} energy parameters for analysis", energy_results.len());
...
fn simulate_batch_write_results(tag_values: &[(&str, PlcValue)]) -> Vec<(String, Result<(), BatchError>)> {
    tag_values.iter().enumerate().map(|(i, (tag, _))| {
...
fn simulate_mixed_batch_results(operations: &[BatchOperation]) -> Vec<rust_ethernet_ip::BatchResult> {
