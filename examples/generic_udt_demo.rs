...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (udt_name, offset, size, data_type, description) in test_cases {
        ...
        match client.read_udt_member_by_offset(udt_name, offset, size, data_type).await {
            Ok(value) => {
    ...
    let write_tests = vec![
        ("Part_Data", 0, 1, 0x00C1, PlcValue::Bool(true), "BOOL at offset 0"),
        ("Part_Data", 4, 4, 0x00CA, PlcValue::Real(99.9), "REAL at offset 4"),
        ("Part_Data", 8, 4, 0x00C4, PlcValue::Dint(12345), "DINT at offset 8"),
        ("MyUDT", 0, 1, 0x00C1, PlcValue::Bool(false), "BOOL at offset 0"),
        ("TestUDT", 0, 84, 0x00CE, PlcValue::String("Hello World".to_string()), "STRING at offset 0"),
    ];
    ...
    for (udt_name, offset, size, data_type, value, description) in write_tests {
        ...
        match client.write_udt_member_by_offset(udt_name, offset, size, data_type, value.clone()).await {
            Ok(_) => {
    ...
    let data_types = vec![
        ...
        (0x00C6, "WORD", PlcValue::Word(0xABCD)),
        (0x00C7, "DWORD", PlcValue::Dword(0x12345678)),
        (0x00C8, "LWORD", PlcValue::Lword(0x123456789ABCDEF0)),
                ...
                match udt.parse_member_value(&member, &serialized) {
                    Ok(parsed_value) => {
                        if parsed_value == test_value {
                            ...
                            println!("  ⚠️ Deserialization mismatch: expected {:?}, got {:?}", test_value, parsed_value);
                        }
                    }
                    Err(e) => {
    ...
    match client.read_udt_member_by_offset("Part_Data", 9999, 1, 0x00C1).await {
    ...
    match client.write_udt_member_by_offset("Part_Data", 0, 1, 0x9999, PlcValue::Bool(true)).await {
    ...
    match client.write_udt_member_by_offset("Part_Data", 0, 1, 0x00C1, PlcValue::Real(3.14)).await {
        Ok(_) => println!("  ❌ Unexpected success"),
        Err(e) => println!("  ✅ Expected error: {}", e),
    }
